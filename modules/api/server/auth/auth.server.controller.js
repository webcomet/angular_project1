...
exports.signup = function (req, res) {
  ...
  var festival= new Festival();
   if (req.body.festivalName && req.body.festivalName !== '') {
  ...
  user.save(function (err, user) {
    if (err) {
      festival.remove({}, function(err, festival) {});
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
    } else {
      ...
      user.salt = undefined;
      if (user.festivals.length) {
        festival.createdBy = user;
